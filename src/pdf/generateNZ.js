...
export async function generateNZ({ config, catalog, managerName, clientName, price, nzNumber, calcNumber }) {
  const model = config.modelId ? catalog.models?.[config.modelId] : null;
  const filename = getNZFilename(model, new Date());

  const doc = await fillNZTemplate({ config, catalog, managerName, clientName, price, nzNumber, calcNumber });
