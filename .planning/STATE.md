...
| # | Требование | Статус |
|---|-----------|--------|
| PERF_1 | Загрузка ≤ 2 сек | ✅ Lighthouse desktop 2026-05-07: FCP 0.5с / LCP 1.1с / TBT 0мс / Score 98 |
...
| REL_1 | Кроссбраузерность Chrome/Firefox/Safari/Edge | ⏳ Отложено — проверим после завершения всех фаз |
...
| # | Вопрос | Статус |
|---|--------|--------|
| 1 | Толщина 0.5 мм — ставка 10% в таблице, не 0%? | ✅ ML default=0.45 → 0.5=10%; LS default=0.5 → 0.5=0% (стандарт). Код корректен: buildThicknessOptions + calcPrice сравнивают с defaults.bodyThickness |
| 2 | Вентиляция от 100 шт. — ставка не заполнена | ✅ Ставки подтверждены 2026-05-07: roof <10=57%/от10=27%/от50=17%/от100=7%; roofBottom <10=61%/от10=31%/от50=21%/от100=11%. Нужно заполнить в Google Sheets строки 36/40 столбец 4, затем sync-sheets.js |
| 3 | Цвет корпуса кат. 3: от 50 шт. = 30%, от 51 шт. = 20% — верно? | ✅ Только одна ставка: от 50 шт = 30%. Подтверждено sync-sheets.js 2026-05-07 |
| 4 | Ширина нестандартная — доплаты нет? | ✅ Верно — ширина считается только экономистом вручную, всегда ПСС. Подтверждено 2026-05-07 |
| 5 | Нестандартная высота/глубина вне таблицы — всегда ПСС? | ✅ Неприменимо — пользователь выбирает только из snap-значений степпера (1800/1850/1860/1900/2000 мм; 300/400/450 мм), произвольный ввод невозможен |
| 6 | Лимит 2 исполнения — с 3-го всегда ПСС? | ✅ Подтверждено 2026-05-07. Код: changeCount > 2 → manual: true |
