...
- [x] **USERS-01**: Администратор видит список всех пользователей системы на отдельной вкладке
- [x] **USERS-02**: Администратор может создать новый аккаунт пользователя (задать email и пароль) прямо из админ-панели
- [x] **USERS-03**: Администратор может удалить (деактивировать) аккаунт пользователя
- [x] **USERS-04**: Создание/удаление пользователей реализовано через Firebase Cloud Function (единственный серверный компонент — Admin SDK недоступен из браузера)

### User History (HIST)

- [x] **HIST-01**: При скачивании КП конфигурация автоматически сохраняется в историю пользователя (Firestore `/users/{uid}/history`)
- [x] **HIST-02**: Пользователь видит список своих скачанных КП (дата, модель, цена)
- [x] **HIST-03**: Пользователь может восстановить сохранённую конфигурацию в конфигуратор одним кликом
- [x] **HIST-04**: Пользователь может повторно скачать КП из истории без ручной настройки
...
| USERS-01 | Phase 11 | Complete |
| USERS-02 | Phase 11 | Complete |
| USERS-03 | Phase 11 | Complete |
| USERS-04 | Phase 11 | Complete |
